...
class Database(object):
    def __init__(self, gtpath=None, dtpath=None, body_key=None, head_key=None, mode=0, if_face=False, body_part='body'):
        """
        mode=0: only body; mode=1: only head
        """
        self.images = dict()
        self.eval_mode = mode
        self.loadData(gtpath, body_key, head_key, True, if_face, body_part)
        self.loadData(dtpath, body_key, head_key, False, if_face, body_part)
    ...
    def loadData(self, fpath, body_key=None, head_key=None, if_gt=True, if_face=False, body_part='body'):
        ...
        if if_gt:
            for record in records:
                img_name = record["ID"][0] if isinstance(record["ID"], list) else record["ID"]
                self.images[img_name] = Image(self.eval_mode)
                self.images[img_name].load(record, body_key, head_key, PERSON_CLASSES, 
                    True, if_face=if_face, body_part=body_part)
        else:
            for record in records:
                img_name = record["ID"][0] if isinstance(record["ID"], list) else record["ID"]
                self.images[img_name].load(record, body_key, head_key, PERSON_CLASSES, 
                    False, if_face=if_face, body_part=body_part)
                self.images[img_name].clip_all_boader()
