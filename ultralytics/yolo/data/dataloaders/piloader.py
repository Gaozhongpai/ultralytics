...
from ultralytics.yolo.data.dataloaders.piaugmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup
from ultralytics.yolo.data.dataloaders.pigeneral import check_requirements, check_file, check_dataset, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, \
...
def labels_to_bodyhand_v2(labels):
    kTargetAngle = math.pi * 0.5
    bh = np.zeros([labels.shape[0], 2])
    for k, lbl in enumerate(labels):
        if lbl[0] == 1:
            xc, yc, w, h, x, y = lbl[1:7].copy()
            x1, y1 = x - xc, y - yc  ## offset from the body to the part
            bh[k, 0] = (NormalizeRadians(kTargetAngle - math.atan2(-y1, x1))) / math.pi
            bh[k, 1] = math.sqrt(x1**2 + y1**2)
    return bh

## by angle
def labels_to_bodyhand_v3(labels):
    kTargetAngle = math.pi * 0.5
    bh = np.zeros([labels.shape[0], 2])
        ...
        if l1[0] != 0: # l1[0] == 1 or 2 for hand
            for l2 in labels[max(k-2, 0):k+3]:
                if (l1[-2:]==l2[-2:]).all() and l2[0]==0:
                    x1, y1 = l2[1:3] - l1[1:3] # offset from the body to the part
                    bh[k, 0] = NormalizeRadians(kTargetAngle - math.atan2(-y1, x1)) / math.pi
                    bh[k, 1] = math.sqrt(x1**2 + y1**2)
    return bh
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        # part_rot = bh[labels[:, 0] == 1]
        # for k, lbl in enumerate(part_obj):
        ...
        #     rot = part_rot[k, 0] * math.pi
        #     lens = part_rot[k, 1]
        #     dx, dy = lens * math.sin(rot), -lens * math.cos(rot)
        #     part_pt = (int((dx + xc) * img_w), int((dy + yc) * img_h))
