...
from ultralytics.yolo.data.dataloaders.pigeneral import LOGGER, check_version, colorstr, resample_segments, segment2box, xywhn2xyxy
...
def random_perspective(im,
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates(cls=targets[:, 0], box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]  # filter out bbox labels which are out-of-range image map
        targets[:, 1:5] = new[i]
...
def box_candidates_v1(cls, box1, box2, wh_thr=10, ar_thr=10, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n)
