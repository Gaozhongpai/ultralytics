...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), 
                                               Conv(c3, c3, 3), 
                                               nn.Conv2d(c3, c4, 1)) for x in ch) # rotation
    ...
    def forward(self, x):
        ...
        box, cls, rot = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2).split((self.reg_max * 4, self.nc, 1), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = torch.cat((dbox, cls.sigmoid(), (rot.sigmoid() - 0.5) * 2), 1)
